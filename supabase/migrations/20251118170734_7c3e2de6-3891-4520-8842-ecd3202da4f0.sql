-- Fix duplicate addiction on user creation
-- Drop and recreate the handle_new_user function to not create duplicate addictions
...
BEGIN
  ...
  -- Insert default goals based on language
  IF user_language = 'en' THEN
    ...
    VALUES
      (NEW.id, 'Check-in 🧘', 'daily', NULL, 1, false, 1, 'en'),
      (NEW.id, 'Emotion Journal 🎭', 'daily', NULL, 1, false, 2, 'en'),
      (NEW.id, 'Gratitude Journal 🙏', 'daily', NULL, 1, false, 3, 'en'),
      (NEW.id, 'Personal Journal 📔', 'weekly', 'weekly', 1, false, 4, 'en');
  ELSE
    -- Spanish goals (default)
    INSERT INTO public.goals (user_id, text, goal_type, periodic_type, remaining, completed, order_index, language)
    VALUES
      (NEW.id, 'Check-in 🧘', 'daily', NULL, 1, false, 1, 'es'),
      (NEW.id, 'Diario de emociones 🎭', 'daily', NULL, 1, false, 2, 'es'),
      (NEW.id, 'Diario de gratitud 🙏', 'daily', NULL, 1, false, 3, 'es'),
      (NEW.id, 'Diario personal 📔', 'weekly', 'weekly', 1, false, 4, 'es');
  END IF;
