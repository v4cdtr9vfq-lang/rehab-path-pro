            ...
            {isMobile ? (
              <Select value={currentRoom} onValueChange={(value) => {
                ...
                <SelectTrigger className="w-full bg-muted text-foreground border-border flex items-center pr-[15px] pl-3 relative [&>svg]:ml-[15px]">
                  ...
                  <Badge 
                    variant="secondary" 
                    className="flex-shrink-0 inline-flex gap-1 text-xs px-1.5 py-0.5 h-5 bg-muted text-black/60 pointer-events-none"
                  >
                  ...
                  {CHAT_ROOMS.map((room) => (
                    <SelectItem key={room.id} value={room.id} className="cursor-pointer pr-[15px] pl-3 [&>span:first-child]:hidden">
                      <span className="flex items-center justify-between w-full pr-[40px]">
                        ...
                        <Badge 
                          variant="secondary" 
                          className="absolute right-[15px] inline-flex gap-1 text-xs px-1.5 py-0.5 h-5 bg-muted text-black/60 pointer-events-none"
                        >
                          <Users className="h-3 w-3" />
                  ...
                  {CHAT_ROOMS.map((room, index) => (
                    ...
                    >
                      <span className="flex items-center justify-between gap-2 w-full">
                        ...
                        <Badge 
                          variant="secondary" 
                          className="inline-flex gap-1 text-xs px-1.5 py-0.5 h-5 bg-muted text-black/60 pointer-events-none"
                        >
                          <Users className="h-3 w-3" />
                          {onlineCountByRoom[room.id] || 0}
                        </Badge>
